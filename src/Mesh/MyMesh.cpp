...
#pragma region Phong Shader
		ShaderInfo shadersPhong[] = {
			...
			{ GL_NONE, NULL } };
		programPhong = LoadShaders(shadersPhong);//讀取shader

		glUseProgram(programPhong);//uniform參數數值前必須先use shader
...
#pragma region Line Shader
		ShaderInfo shadersLine[] = {
			...
			{ GL_NONE, NULL } };
		programLine = LoadShaders(shadersLine);//讀取shader

		glUseProgram(programLine);//uniform參數數值前必須先use shader
